...
* A small value of K will give a more flexible fit, which has low bias but high variance. As mentioned above, K=1 will give point-to-point plots, which is an overfit. At K=2 we get a lower variance, and there is no sign of underfitting the curve. At K=10 the variance is notably lower, but the plot shows signs of underfitting and a low flexibility. Continuing the trend, at K=25 we get clearly a underfitted and inflexible result.
...
* You start by fitting a curve to the training data sets by the KNN method. Then you can express the fittet curve as $Y = f(x) + \epsilon$. Here $\epsilon$ is the random or irreducible error, and we assume it has mean zero and constant variance equal to $\sigma^2$. The expected test mean squared error (MSE) for $x_0$ is defined as:
...
* Interpretation of Figure 4:
    + The squared bias decreases as the flexible increases. That means that for high flexibility there is no systematic difference from the predicted model and the values being estimated. At higher values for K there will be a systmatic difference. As example, in a) when K=61, we will always get a straight line, and differs from the true values.
    + The variance decreases as K increases, which is due to more points taken account for, and less difference from each training set to the others.
    + The irreducible error isn't dependent on the values of K, and is therefore constant.
    
* Based on the sum of the errors, the lowest error is K=3 (but nearly the same as K=5). Thus this will be our choice for K as this will give the most accurate model. This agrees fairly well with what we found in a), and our conclusion would be that a K between 3 and 5 would give the best results.
...
The plot of fitted values vs residuals for modelB seems quite similar to the one for modelA. However, although the residuals mostly are spread around a horizontal line, it is seemingly not the case for all the largest and smalles fitted values. It is however difficult to state whether this indicates that the assumption of linear relationship between the response and the covariates does not hold. Also, the residuals in the qq-plot do not quite follow a straight line, so the residuals are probably not Gaussian distributed. 
...
This test tells us how well the data follow the Gaussian distribution. The better the distribution fits the data, the smaller the p-value will be. From the p-values here, we can clearly conclude that the residuals in modelB are not Gaussian distributed, while the opposite is most likely true in modelA.
...
lda_manual=ggplot(train, aes(x=x1, y=x2, color=y))+geom_point(size=2.5) + geom_point(data=test, shape=4)+ geom_abline(slope=slope1, intercept=intercept1) + ggtitle("Train and test and logistic boundary")
lda_manual
